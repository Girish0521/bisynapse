...
function LoginContent() {
  ...
  const handleCompleteLogin = (role: UserRole, userEmail?: string) => {
    ...
    setTimeout(() => {
      if (typeof window !== 'undefined') {
        ...
        localStorage.setItem('bisynapse_user_email', userEmail || `${role}@bisynapse.gov.in`);
      }
  ...
  const handleFormLogin = (e: React.FormEvent) => {
    ...
    handleCompleteLogin(selectedRole, email);
  };
    ...
    <div className="max-w-2xl w-full space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-slate-600 font-medium">
          Select how you want to use BISynapse
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm space-y-6">
            ...
            {roles.map((r) => {
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-lg border text-left transition-all flex items-start space-x-3 ${isSelected ? 'bg-[#0F4C81] text-white border-[#0A2540] shadow-sm' : 'bg-slate-50 hover:bg-slate-100 text-slate-800 border-slate-200'}`}
                >
                  <div className={`w-8 h-8 rounded flex items-center justify-center shrink-0 ${isSelected ? 'bg-white/20 text-amber-300' : 'bg-slate-200 text-[#0F4C81]'}`}>
        ...
        <div className="pt-4 border-t border-slate-200 space-y-4">
          ...
          <button
            ...
            className="w-full py-2.5 bg-white hover:bg-slate-50 text-slate-700 font-bold text-xs rounded-lg border border-slate-300 shadow-2xs transition-colors flex items-center justify-center space-x-2 disabled:opacity-50"
          >
